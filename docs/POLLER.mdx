...
sequenceDiagram
    ...
    participant Fn as Anonymous Function
    ...
    Note over P: Runs indefinitely<br/>until app exits
    
    style PH fill:#667eea,color:#fff
...
```mermaid
graph LR
    A[Poller<br/>1s Timer] -->|poll| B[GetPlayerInfo<br/>playerctl]
    B -->|media data| C[Channel<br/>Buffer: 100]
    C -->|broadcast| D[WebSocket<br/>Writer]
    D -->|JSON| E[Web Client]
